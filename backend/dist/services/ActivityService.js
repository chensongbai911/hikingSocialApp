...
export class ActivityService {
    ...
    async generateActivityId() {
        const [rows] = await pool.query('SELECT id FROM activities ORDER BY id DESC LIMIT 1');
        if (rows.length === 0) {
            return 'act-001';
        }
    ...
    async generatePhotoId() {
        const [rows] = await pool.query('SELECT id FROM activity_photos ORDER BY id DESC LIMIT 1');
        if (rows.length === 0) {
            return 'aphoto-001';
        }
        const lastId = rows[0].id;
    ...
    async saveActivityPhotos(activityId, photos) {
        ...
        for (let i = 0; i < photos.length && i < 6; i++) {
            ...
            await pool.query(`INSERT INTO activity_photos (id, activity_id, photo_url, is_cover, sort_order, created_at)
    ...
    async getActivityPhotos(activityId) {
        const [photos] = await pool.query(`SELECT photo_url FROM activity_photos
       WHERE activity_id = ?
    ...
    async generateParticipationId() {
        const [rows] = await pool.query('SELECT id FROM participations ORDER BY id DESC LIMIT 1');
        if (rows.length === 0) {
            return 'part-001';
        }
        const lastId = rows[0].id;
    ...
    async createActivity(creatorId, data) {
        ...
        await pool.query(`INSERT INTO activities (
    ...
    async getActivityById(activityId, currentUserId) {
       ...
       WHERE a.id = ? AND a.deleted_at IS NULL`, [activityId]);
        ...
        let isJoined = false;
        if (currentUserId) {
            const [participation] = await pool.query('SELECT id FROM participations WHERE user_id = ? AND activity_id = ? AND status = "joined"', [currentUserId, activityId]);
    ...
    async updateActivity(activityId, creatorId, data) {
        // 验证权限
        const [activities] = await pool.query('SELECT creator_id FROM activities WHERE id = ? AND deleted_at IS NULL', [activityId]);
        if (activities.length === 0) {
            throw {
                code: BusinessErrorCode.RESOURCE_NOT_FOUND,
                message: '活动不存在'
        ...
        if (updates.length > 0) {
            ...
            await pool.query(`UPDATE activities SET ${updates.join(', ')} WHERE id = ? AND deleted_at IS NULL`, values);
        }
        return this.getActivityById(activityId, creatorId);
    }
    /**
    ...
    async deleteActivity(activityId, creatorId) {
        ...
        await pool.query('UPDATE activities SET deleted_at = NOW(), status = "cancelled" WHERE id = ?', [activityId]);
    }
    /**
     * 加入活动
     */
    async joinActivity(activityId, userId) {
        // 检查活动是否存在
        const [activities] = await pool.query('SELECT id, creator_id, max_participants, status FROM activities WHERE id = ? AND deleted_at IS NULL', [activityId]);
        if (activities.length === 0) {
            throw {
                code: BusinessErrorCode.RESOURCE_NOT_FOUND,
                message: '活动不存在'
        ...
        // 检查是否已加入
        const [existing] = await pool.query('SELECT id FROM participations WHERE user_id = ? AND activity_id = ? AND status = "joined"', [userId, activityId]);
        ...
        if (activity.max_participants) {
            const [countResult] = await pool.query('SELECT COUNT(*) as count FROM participations WHERE activity_id = ? AND status = "joined"', [activityId]);
        ...
        await pool.query(`INSERT INTO participations (id, user_id, activity_id, status, joined_at, created_at)
       VALUES (?, ?, ?, 'joined', NOW(), NOW())`, [participationId, userId, activityId]);
        const [participations] = await pool.query('SELECT * FROM participations WHERE id = ?', [participationId]);
    ...
    async leaveActivity(activityId, userId) {
        const [result] = await pool.query(`UPDATE participations
