...
export class DestinationService {
    ...
    // 获取目的地列表
    async getDestinations(filters, userId) {
        try {
            ...
            // 关键词搜索
            if (filters?.keyword) {
                whereConditions.push('(d.name LIKE ? OR d.description LIKE ? OR d.tags LIKE ? OR d.area LIKE ?)');
        ...
        LIMIT ${limit} OFFSET ${offset}`);
            ...
            const destinations = rows.map((row) => {
                ...
                if (filters?.latitude && filters?.longitude) {
                    dest.user_distance = this.calculateDistance(filters.latitude, filters.longitude, dest.latitude, dest.longitude);
                }
                return dest;
            });
            // 如果按距离排序
    ...
    async getDestinationById(id, userId) {
        try {
          ...
          0 as is_favorited
        FROM destinations d
        WHERE d.id = ? AND d.status = 'active'`, [id]);
            if (rows.length === 0)
                return null;
            ...
            if (destination.photos) {
                try {
                    destination.photos = JSON.parse(destination.photos);
                }
                catch {
                    destination.photos = [];
                }
    ...
    // 获取热门目的地
    async getPopularDestinations(limit = 10, userId) {
        try {
    ...
    async getNearbyDestinations(latitude, longitude, radius = 50, limit = 10, userId) {
        ...
        WHERE d.status = 'active'`, []);
            ...
            const destinations = rows
                ...
                dest.user_distance = this.calculateDistance(latitude, longitude, dest.latitude, dest.longitude);
                return dest;
            })
