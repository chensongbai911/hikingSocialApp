...
export class MessageService {
    ...
    async getOrCreateConversation(userId1, userId2) {
        ...
        try {
         ...
         FROM conversations
         WHERE user_id1 = ? AND user_id2 = ? AND deleted_at IS NULL`, [minId, maxId]);
    ...
    async sendMessage(conversationId, senderId, content, contentType = 'text', imageUrl, fileUrl) {
        ...
        if (String(conversation.userId1) !== String(senderId) &&
        ...
        // 更新对话的最后消息信息
        const otherUserId = conversation.userId1 === senderId
            ? conversation.userId2
            : conversation.userId1;
        await conversation.update({
            lastMessageId: message.id,
            lastMessageAt: new Date(),
            lastMessageContent: contentType === 'text'
                ? content.substring(0, 100)
                : `[${contentType === 'image' ? '图片' : '文件'}]`,
            [conversation.userId1 === senderId
                ? 'user2UnreadCount'
                : 'user1UnreadCount']: (conversation[conversation.userId1 === senderId
                ? 'user2UnreadCount'
                : 'user1UnreadCount'] || 0) + 1,
        });
        // 返回完整消息对象（包括发送者信息）
    ...
    async markConversationAsRead(conversationId, userId) {
       ...
       WHERE id = ? AND deleted_at IS NULL`, [conversationId]);
        ...
        // 检查用户是否是对话参与者
        if (String(conversation.user_id1) !== String(userId) &&
    ...
    async getUnreadCount(userId) {
        try {
          ...
          END) as totalUnread
        FROM conversations
        WHERE (user_id1 = ? OR user_id2 = ?)`, [userId, userId, userId, userId]);
