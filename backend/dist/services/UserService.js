...
export class UserService {
    ...
    async getProfile(userId) {
        // 获取用户基本信息
        const [users] = await pool.query(`SELECT id, email, nickname, gender, age, avatar_url, bio, hiking_level, is_verified, created_at
    ...
    async updatePreferences(userId, preferences) {
        ...
        try {
            ...
            // 获取当前最大ID以生成新ID
            const [idRows] = await connection.query('SELECT id FROM user_preferences ORDER BY created_at DESC LIMIT 1');
            let nextNum = 1;
            if (idRows.length > 0) {
