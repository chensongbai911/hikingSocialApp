 ...
 */
import { HttpStatusCode, BusinessErrorCode, ErrorMessages } from '../types/api.types';
/**
 * 成功响应
 ...
 */
export function success(res, data, message = '操作成功', statusCode = HttpStatusCode.OK) {
    const response = {
        code: statusCode,
        message,
        data,
 ...
 * @param message 响应消息
 */
export function created(res, data, message = '创建成功') {
 ...
 */
export function error(res, message, statusCode = HttpStatusCode.BAD_REQUEST, error, details) {
    const response = {
        code: statusCode,
        message,
        error,
 ...
 */
export function businessError(res, businessCode, customMessage, details) {
 ...
 * @param message 响应消息
 */
export function paginated(res, items, page, pageSize, total, message = '查询成功') {
...
export function validationError(res, errors) {
    businessError(res, BusinessErrorCode.VALIDATION_ERROR, '参数验证失败', errors);
}
/**
 * 未授权错误响应
 * @param res Express Response对象
 * @param message 错误消息
 */
export function unauthorized(res, message = '未授权访问') {
 ...
 * @param message 错误消息
 */
export function forbidden(res, message = '权限不足') {
 ...
 */
export function serverError(res, message = '服务器内部错误', errorDetails) {
    // 生产环境不返回错误详情
