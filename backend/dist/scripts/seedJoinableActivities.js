...
async function ensureDemoUser() {
    const [rows] = await database_1.pool.query('SELECT id FROM users WHERE email = ? AND deleted_at IS NULL LIMIT 1', [DEMO_EMAIL]);
    if (rows.length > 0) {
        return rows[0].id;
    }
    const userId = (0, crypto_1.randomUUID)();
    const passwordHash = await bcryptjs_1.default.hash(DEMO_PASSWORD, 10);
    await database_1.pool.query(`INSERT INTO users (
...
async function insertActivities(creatorId) {
    for (const item of seedActivities) {
        const [exists] = await database_1.pool.query('SELECT id FROM activities WHERE id = ? AND deleted_at IS NULL LIMIT 1', [item.id]);
        if (exists.length > 0) {
...
main();
//# sourceMappingURL=seedJoinableActivities.js.map