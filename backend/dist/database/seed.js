...
async function runSQLFile(sqlPath) {
    ...
    try {
        ...
        console.log(`✓ Executed: ${path_1.default.basename(sqlPath)}`);
    }
    catch (err) {
        await connection.rollback();
        console.error(`✗ Failed: ${path_1.default.basename(sqlPath)} ->`, err.message);
        throw err;
    }
    finally {
        connection.release();
...
async function main() {
    try {
        const createFile = path_1.default.join(__dirname, 'create_applications_friendships.sql');
        const sqlFile = path_1.default.join(__dirname, 'seed_v1_5_0.sql');
