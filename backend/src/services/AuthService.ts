    ...
    try {
      ...
      if (includePreferences) {
        ...
        if (optionalResults.length > preferencesIndex) {
          const preferences = optionalResults[preferencesIndex][0] as any[];
          (user as any).preferences = preferences;
        }
      }
