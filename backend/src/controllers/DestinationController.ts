...
export class DestinationController {
  ...
  async getDestinations(req: Request, res: Response) {
    try {
      ...
      const filters = {
        ...
        sortBy: sortBy as 'distance' | 'popularity' | 'rating' | 'activity_count',
      ...
      const { destinations, total } = await DestinationService.getDestinations(
        filters,
        userId
      );
  ...
  // 获取附近目的地
  async getNearbyDestinations(req: Request, res: Response) {
    try {
      const userId = (req as any).user?.id;
  ...
  // 记录搜索
  async recordSearch(req: Request, res: Response) {
    try {
      const userId = (req as any).user?.id;
      if (!userId) {
