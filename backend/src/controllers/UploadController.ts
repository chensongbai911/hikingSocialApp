  ...
  async uploadPhotos(req: Request, res: Response) {
    try {
      ...
      const [countResult] = await pool.query<RowDataPacket[]>(
        'SELECT COUNT(*) as count FROM user_photos WHERE user_id = ? AND deleted_at IS NULL',
        [userId]
      );
      ...
      if (currentCount + files.length > maxPhotos) {
        ...
        return businessError(
          res,
          BusinessErrorCode.MAX_PHOTOS_EXCEEDED,
          `最多只能上传${maxPhotos}张照片，当前已有${currentCount}张`
        );
      }
      ...
      for (const file of files) {
        try {
          ...
          // 生成照片ID
          const [lastPhoto] = await pool.query<RowDataPacket[]>(
          ...
          await pool.query(
            'INSERT INTO user_photos (id, user_id, photo_url, thumbnail_url, display_order) VALUES (?, ?, ?, ?, ?)',
            [photoId, userId, photoUrl, thumbnailUrl, currentCount + uploadedPhotos.length + 1]
          );
