...
export class MessageController {
  ...
  static async recallMessage(
    ...
    try {
      ...
      const [rows] = await pool.query<any[]>(
        'SELECT id, sender_id, created_at, is_recalled FROM messages WHERE id = ? LIMIT 1',
        [messageId]
      )
      ...
      await pool.query(
        'UPDATE messages SET is_recalled = TRUE, recalled_at = NOW(), recalled_by = ? WHERE id = ?',
        [userId, messageId]
      )

      // 推送撤回事件给双方
      const [convRows] = await pool.query<any[]>(
        'SELECT conversation_id FROM messages WHERE id = ? LIMIT 1',
        [messageId]
      )
  ...
  static async reportMessage(
    ...
    try {
      ...
      await pool.query(
        'INSERT INTO message_reports (message_id, reporter_id, reason, extra) VALUES (?, ?, ?, ?)',
        [messageId, String(userId), reason, extra ? JSON.stringify(extra) : null]
      )
  ...
  static async clearConversation(
    ...
    try {
      ...
      try {
        ...
        // 将现有消息归档（逐条快照）
        await conn.query(
          `INSERT INTO messages_archive (conversation_id, original_message_id, snapshot, archived_reason)
